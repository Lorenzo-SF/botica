defmodule Botica.Doctor do
  ...
  alias Botica.Doctor.FlagsSummary
  ...
  Delegates to `Botica.Doctor.FlagsSummary.snapshot/0`.
  """
  @spec flags_summary() :: %{
          required(:count) => non_neg_integer(),
          required(:flags) => [map()]
        }
  def flags_summary, do: FlagsSummary.snapshot()

  @doc """
  Formats the `flags_summary/0` output as a human-readable string.
  Delegates to `Botica.Doctor.FlagsSummary.format/0`.
  """
  @spec format_flags_summary() :: String.t()
  def format_flags_summary, do: FlagsSummary.format()
